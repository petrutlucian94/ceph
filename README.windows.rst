...
When performing mapping operations (e.g. creating, removing, listing or showing
mappings), using the ``rbd-nbd`` command directly is recommended, which avoids
an extra call and exposes all the available parameters.
