...
class RbdImage(object):
    ...
    def import_image(cls, name: str, path: str, export_format: int = 1):
        ...
        cmd = ["rbd", "import", "--export-format", str(export_format), path, name]
        utils.ps_execute(*cmd)
    ...
    def map(self, timeout: int = 60):
        ...
        utils.ps_execute("rbd-wnbd", "map", self.name)
        self.mapped = True
    ...
    def refresh_after_remap(self, timeout: int = 60):
        ...
        # The disk number is expected to change after a remap.
        self.disk_number = self.get_disk_number(timeout=timeout)
    ...
    def unmap(self):
        if self.mapped:
            ...
            utils.ps_execute("rbd-wnbd", "unmap", self.name)
            self.mapped = False
    ...
    def remove(self):
        if not self.removed:
            ...
            utils.ps_execute("rbd", "rm", self.name)
            self.removed = True
