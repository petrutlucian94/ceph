...
std::string AdminSocket::destroy_wakeup_pipe()
{
  ...
  // Close write end
  retry_sys_call(::rpl_closesocket, m_wakeup_wr_fd);
  m_wakeup_wr_fd = -1;
  ...
  // joining.
  retry_sys_call(::rpl_closesocket, m_wakeup_rd_fd);
  m_wakeup_rd_fd = -1;
...
void AdminSocket::do_accept()
{
  ...
  while (1) {
    ...
    if (ret <= 0) {
      ...
      retry_sys_call(::rpl_closesocket, connection_fd);
      return;
    }
    if (cmd[0] == '\0') {
      // old protocol: __be32
  ...
  if (ret < 0) {
    ...
    #ifndef _WIN32
    int r = out.write_fd(connection_fd);
    #else
...
void AdminSocket::shutdown()
{
  ...
  retry_sys_call(::rpl_closesocket, m_sock_fd);
