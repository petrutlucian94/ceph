...
void ClassHandler::shutdown()
{
  for (auto& cls : classes) {
    if (cls.second.handle) {
      close_shared_lib(cls.second.handle);
    }
  }
  classes.clear();
}
...
int ClassHandler::_load_class(ClassData *cls)
{
  ...
  if (cls->status == ClassData::CLASS_UNKNOWN ||
    ...
    cls->handle = open_shared_lib(fname);
    if (!cls->handle) {
      struct stat st;
      int r = ::stat(fname, &st);
      if (r < 0) {
    ...
    cls_deps_t *(*cls_deps)();
    cls_deps = (cls_deps_t *(*)())find_symbol(cls->handle, "class_deps");
    if (cls_deps) {
  ...
  // initialize
  void (*cls_init)() = (void (*)())find_symbol(cls->handle, "__cls_init");
  if (cls_init) {
