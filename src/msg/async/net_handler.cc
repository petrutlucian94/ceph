...
int NetHandler::generic_connect(const entity_addr_t& addr, const entity_addr_t &bind_addr, bool nonblock)
{
  ...
  if (ret < 0) {
    ret = SOCK_ERRNO;
    // TODO: define some macros to check those states.
    #ifdef _WIN32
    if (SOCK_ERRNO == WSAEWOULDBLOCK && nonblock)
    #else
    if (SOCK_ERRNO == EINPROGRESS && nonblock)
    #endif
      return s;
  ...
  ldout(cct, 10) << __func__ << " ----- connect passed. " << dendl;
...
int NetHandler::reconnect(const entity_addr_t &addr, int sd)
{
  ...
  #ifdef _WIN32
  if (ret< 0 && SOCK_ERRNO != WSAEISCONN) {
  #else
  if (ret < 0 && SOCK_ERRNO != EISCONN) {
  #endif
    r = SOCK_ERRNO;
    ldout(cct, 10) << __func__ << " reconnect: " << r
                   << " " << strerror(r) << dendl;
    #ifdef _WIN32
    if (r == WSAEWOULDBLOCK || r == WSAEALREADY)
    #else
