...
{
  ClsLockType lock_type = CLS_LOCK_NONE;
  string tag;
  lockers.clear();
...
TEST(ClsLock, TestMultiLocking) {
  ...
  ClsLockType lock_type_shared = CLS_LOCK_SHARED;
  ClsLockType lock_type_exclusive = CLS_LOCK_EXCLUSIVE;
...
TEST(ClsLock, TestSetCookie) {
  ...
  librados::ObjectWriteOperation op1;
  set_cookie(&op1, name, CLS_LOCK_SHARED, cookie, tag, new_cookie);
  ASSERT_EQ(-ENOENT, ioctx.operate(oid, &op1));

  librados::ObjectWriteOperation op2;
  lock(&op2, name, CLS_LOCK_SHARED, cookie, tag, "", utime_t{}, 0);
  ASSERT_EQ(0, ioctx.operate(oid, &op2));

  librados::ObjectWriteOperation op3;
  lock(&op3, name, CLS_LOCK_SHARED, "cookie 2", tag, "", utime_t{}, 0);
  ASSERT_EQ(0, ioctx.operate(oid, &op3));

  librados::ObjectWriteOperation op4;
  set_cookie(&op4, name, CLS_LOCK_SHARED, cookie, tag, cookie);
  ASSERT_EQ(-EBUSY, ioctx.operate(oid, &op4));

  librados::ObjectWriteOperation op5;
  set_cookie(&op5, name, CLS_LOCK_SHARED, cookie, "wrong tag", new_cookie);
  ASSERT_EQ(-EBUSY, ioctx.operate(oid, &op5));

  librados::ObjectWriteOperation op6;
  set_cookie(&op6, name, CLS_LOCK_SHARED, "wrong cookie", tag, new_cookie);
  ASSERT_EQ(-EBUSY, ioctx.operate(oid, &op6));

  librados::ObjectWriteOperation op7;
  set_cookie(&op7, name, CLS_LOCK_EXCLUSIVE, cookie, tag, new_cookie);
  ASSERT_EQ(-EBUSY, ioctx.operate(oid, &op7));

  librados::ObjectWriteOperation op8;
  set_cookie(&op8, name, CLS_LOCK_SHARED, cookie, tag, "cookie 2");
  ASSERT_EQ(-EBUSY, ioctx.operate(oid, &op8));

  librados::ObjectWriteOperation op9;
  set_cookie(&op9, name, CLS_LOCK_SHARED, cookie, tag, new_cookie);
