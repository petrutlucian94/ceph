  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  if (!S_ISDIR(stbuf.stx_mode)) {
    dout(2) << __func__ << " " << path
            << " failed. Not a directory." << dendl;
    return STATUS_NOT_A_DIRECTORY;
  }

  pfd_context fdc = (pfd_context) &(DokanFileInfo->Context);
  if (NTSTATUS st = do_open_file(path, O_RDONLY, 0755, fdc))
    return st;
  ...
  *fdc = { 0 };
  ...
  if (!ret) { /* File Exists */
    ...
    switch (CreationDisposition) {
      case CREATE_NEW:
        // create & return 0
        return do_open_file(path, O_CREAT | O_RDWR | O_EXCL, 0755, fdc, true);
      case CREATE_ALWAYS:
        // create & return 0
        return do_open_file(path, O_CREAT | O_TRUNC | O_RDWR, 0755, fdc, true);
      case OPEN_ALWAYS:
        return do_open_file(path, O_CREAT | O_RDWR, 0755, fdc, true);
      case OPEN_EXISTING:
        if (path == "/")
...
static NTSTATUS WinCephReadFile(
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  if (Offset > CEPH_DOKAN_MAX_FILE_SZ || Offset < 0 ||
