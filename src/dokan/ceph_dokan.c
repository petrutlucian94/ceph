...
WinCephCreateFile(
  ...
  PDOKAN_FILE_INFO DokanFileInfo) {
  // TODO: remove this.
  DbgPrintW(L">>> CephCreateFile called [%ls].\n", FileName);
  int ret_val = WinCephCreateFileDbg(
    FileName, SecurityContext, DesiredAccess, FileAttributes, ShareMode,
    CreateDisposition, CreateOptions, DokanFileInfo);
  DbgPrintW(L">>> CephCreateFile returned %d [%ls].\n", ret_val, FileName);
  return ret_val;
}
  ...
  PDOKAN_FILE_INFO    DokanFileInfo)
{
  ...
  // TODO: A few notes on logging. We should avoid using different log calls
  // for what should be the same message. Dokany uses multiple threads,
  // in which case messages from different operations can get mixed up.
  //
  // Also, we should probably switch to native ceph logging. The only downside
  // to that is that we're probably no longer able to write directly to a
  // debugger (not sure how important that is though).
  // Dokany will still write to stderr or a debugger though, which would be
  // fine for debug messages but we won't get any error messages to a file.
  ...
  PDOKAN_FILE_INFO  DokanFileInfo)
{
  ...
  // Dokany is handling file locking for us unless we set
  // DOKAN_OPTION_FILELOCK_USER_MODE. Still, this means that the locks
  // will not be propagated to ceph.
  ...
  PDOKAN_FILE_INFO  DokanFileInfo)
{
  ...
  if(ret){
    ...
    // TODO: check if this needs to be propagated.
    return 0;
  }
  ...
  PDOKAN_FILE_INFO  DokanFileInfo)
{
  ...
  // TODO: we should probably use the subdir if specified.
