...
int map_device_using_suprocess(std::string arguments)
{
  ...
  exit_code = get_exe_path(exe_path);
  if (exit_code) {
    goto finally;
  }
...
exit:
    ...
    // TODO: we have to support Windows server 2016. Unix sockets only
    // work on WS 2019, so we can't use the Ceph admin socket abstraction.
